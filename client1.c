...
int main()
{
	...
	{
		if ((c1udpsockfd = socket(p->ai_family, p->ai_socktype,p->ai_protocol)) == -1) 
		{
	...
	for(i=0;i<12;i++) //Searching for corresponding key
	{
		if((strcmp(cd2[i].term,str))==0)
		{
			...
			printf("\nThe Client 1 has received a request with search word %s, which maps to key %s.\n",str, skey);
		}
	}
	...
	if ((numbytes = sendto(c1udpsockfd, s1msg, strlen(s1msg), 0, p->ai_addr, p->ai_addrlen)) == -1) 
	{
	...
	if (getsockname(c1udpsockfd, (struct sockaddr *)&udpaddr, &length2) == -1)
	{
		perror("getsockname");
		exit(1);
	}
	...
	printf("\nThe Client 1 sends the request %s to the  Server 1 with port number %s and IP address %s.", s1msg, UDPPORT, s) ;
	freeaddrinfo(servinfo);
    
    //display c2 address
	int client_udpport = ntohs(udpaddr.sin_port);
	inet_ntop(p->ai_family, get_in_addr((struct sockaddr *)p->ai_addr), s2, sizeof s2);
	printf("\nThe Client 1's port number is %d and IP address is %s.\n", client_udpport, s2);
	
	numbytes = strlen(s1msg) + 2;
	
	addr_len = sizeof their_addr;
	if ((numbytes = recvfrom(c1udpsockfd, s1msg, MAXBUFLEN-1 , 0, (struct sockaddr *)&their_addr, &addr_len)) ==-1)
	{
		perror("recvfrom");
		exit(1);
	}
	...
	printf("\nThe Client 1 received the value %s from the Server 1 with port number %s & IP address %s.", VSearch , UDPPORT, s);
	printf("\nThe Client 1's port number is %d and IP address is %s.\n", client_udpport, s2);
